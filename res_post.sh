...
python -c "import optuna; optuna.create_study(storage='sqlite:///Resilient_def.db', study_name='Resilient_post', load_if_exists=True)"
#python -c "import optuna; optuna.create_study(storage='sqlite:///Susceptible_def.db', study_name='Susceptible_post', load_if_exists=True)"
echo "Databases initialized"

echo "Launching workers..."

# Mouse A: Resilient (2 Workers)
for i in $(seq 1 2);
do
	nice -n 10 python fit_after.py --mouse Resilient > log_Res_${i}.txt 2>&1 &
done
